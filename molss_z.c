...
void walk() {
	...
	int x0[NHOLES]={0}, y0[NHOLES]={0}, z0[NHOLES], count[NHOLES]={0};
		...
		for(i=-3; i<=3; i+=3) {
			for(j=-3; j<=3; j+=3) {
				if(M[x+i][y+j][z] >= 2) {    //found a hole!
					...
					for(r=0; r<=index; r++) {
						if((x+i)==x0[r] && (y+j)==y0[r] && z==z0[r]) {   //same as a previous hole?
							count[r]++;
							break;
						}							
					}

					if((x+i)!=x0[r] || (y+j)!=y0[r] || z!=z0[r] || count[r]>7) {  //new hole or old counted hole
						fillIn=0;
						if((x+i)!=x0[r] || (y+j)!=y0[r] || z!=z0[r]) { //new hole
							index++;
							count[index] = 0;
							x0[index] = x+i; y0[index] = y+j; z0[index] = z;     //save position of hole
